...
exports.updatebankQr = async (req, res) => {
    try {
        ...
        let bankDetails = await bankDetailsModel.findOne();
        if (bankDetails) {
            bankDetails.qrCode = qrCodePath;
            await bankDetails.save();
            return res.json({ success: true, message: "QR Code updated successfully", qrCode: qrCodePath });
        } else {
            await bankDetailsModel.create({ qrCode: qrCodePath });
            return res.json({ success: true, message: "QR Code saved successfully", qrCode: qrCodePath });
        }
    } catch (error) {
